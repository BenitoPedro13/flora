...
| Concern | Choice |
|---|---|
| `apps/web`, `apps/api`, `apps/worker` | **Amended 2026-08-17 (`TASK-railway-deploy`).** All three as Railway services in one project, not split across Vercel + Railway/Fly — user's explicit call. `apps/web` and `apps/api` each get a Railway-generated build/start command (`pnpm install --frozen-lockfile && pnpm exec turbo run build --filter=<app>...` / `pnpm --filter <app> run start:prod`), `apps/api` also gets `packages/db`'s migration script as a `preDeployCommand`. Provisioned via plain `railway add`/`railway variable set`/the GraphQL API directly (`railway api`), not `railway config apply` (`.railway/railway.ts` — kept in the repo as topology documentation, but its image- and GitHub-sourced resources never got a working `ServiceInstance` in this account; see that task's §2/§7 for the full failure mode) |
| Postgres | `postgis/postgis:16-3.4` deployed as its own Railway service from a Docker image, not Railway's managed Postgres plugin (plain `postgres:16`, no PostGIS extension available) — same image family `infra/docker-compose.yml` runs locally. `PGDATA` must point at a subdirectory of the mounted volume (`/var/lib/postgresql/data/pgdata`), not the mount point itself — `initdb` refuses a non-empty mount point (its `lost+found` dir) as the data directory (`TASK-railway-deploy` §2) |
| Redis | Railway's managed Redis plugin |
| Object storage | **Amended 2026-08-17.** Self-hosted `minio/minio` as a Railway service, not Cloudflare R2 (the user didn't have R2 credentials for this deploy) and not Railway's own native `bucket()` resource (Tigris-backed, confirmed private-only — no public bucket policy, `PutBucketPolicy`/`PutBucketAcl` don't work, presigned URLs are the only documented access path — which violates invariant 2). MinIO mirrors `infra/docker-compose.yml`'s `storage`/`createbuckets` pair exactly: `mc mb` + `mc anonymous set download` for a real, permanent public-read URL |
| Migrations | drizzle-kit, run as a release step (Railway's `preDeployCommand`), never on app boot |
| Secrets | `CDSE_CLIENT_ID/SECRET`, `MAPBOX_TOKEN`, `DATABASE_URL` (the `flora_app` role, §10 — rotated off the migration's hardcoded dev password immediately after first deploy, `packages/db/migrations/0003_tenancy_rls.sql`'s own instruction), `DATABASE_MIGRATION_URL` (the owner role — release step / one-off tooling only, never a running app), `REDIS_URL`, `S3_*`, `JWT_SIGNING_KEY` (≥32 bytes) |
| Cookies / same-origin | **Resolved 2026-08-15 (`TASK-auth-tenancy`), reconfirmed provider-agnostic 2026-08-17.** The `SameSite=Lax` design was never actually about which provider hosts which app — `apps/web`'s `next.config.ts` `rewrites()` proxies `/api/v1/*` to `API_URL` (server-side only, never `NEXT_PUBLIC_`) so the browser only ever sees `apps/web`'s origin regardless of where `apps/api` physically runs. Putting all three apps on Railway (above) needed no change to this design, only to the doc's provider column. Rejected: `SameSite=None; Secure` + a CORS allowlist + CSRF tokens — materially more surface for the same outcome. |
| Next.js config-as-code caveat | **Found 2026-08-17 (`TASK-railway-deploy`).** Railway's Railpack build sandbox does not expose service environment variables to the build command, even though the same variables resolve correctly via `railway variable list` — confirmed empirically, contradicts Railway's own general docs. `apps/web/next.config.ts`'s `rewrites()` needs `API_URL` at build time regardless (Next.js bakes rewrites into `.next/routes-manifest.json` once, at `next build`; `next start` does not re-invoke `rewrites()`), so the config now exports a function of `phase` and falls back to the deployment's known private-network URL only during `PHASE_PRODUCTION_BUILD`. |
