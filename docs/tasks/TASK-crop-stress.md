...
| `TASK-satellite-live` (**landed 2026-08-16**) | `TASK-satellite-pipeline` §6 item 1 (the live CDSE round trip) — closed; items 12 and 13 (NFR-5, PU measurement) stay open. **All three findings recorded below were resolved live** (`TASK-satellite-live` §1.2): **1.** *Misattributed.* The `Content-Type was not one of "multipart/form-data" or "application/x-www-form-urlencoded"` error was never from `token.ts`/Keycloak — `token.ts` returns a real token every time, verified live. The string is **undici's own** `Response.formData()` parser error, thrown at `process.ts`'s old `res.formData()` call when handed a bare TIFF; it only *looks* like a server error because it names the two content types a server would accept. **2.** *No change needed.* `/api/v1/process` (as coded) and the docs' `/process/v1` are live aliases, byte-identical. **3.** *Confirmed, and the root cause.* With `Accept: application/tar` CDSE returns a real TAR archive (`Content-Type: application/x-tar`), members named `<identifier>.tif`; without that header it silently collapses to a single bare `image/tiff` (`scl` dropped, no error) — which is what the old `res.formData()` call choked on. Fixed in `process.ts` with `nanotar` and a content-type guard; see `TASK-satellite-live` for the full fix and its real captured fixtures |
